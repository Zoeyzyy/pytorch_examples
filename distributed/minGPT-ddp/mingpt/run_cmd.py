...
with open("cmd.sh", "w") as file:
    ...
    for ip in all_ip:
        # 向文件中写入内容
        if(ip != local_ip):
            file.write("nohup tcpdump host 172.22.5." + str(ip) + " -s 128  -w ./packet/from" + str(ip) +".pcap & \n")
            file.write("nohup tcpdump dst 172.22.5." + str(ip) + " -s 128  -w ./packet/to" + str(ip) +".pcap & \n")
