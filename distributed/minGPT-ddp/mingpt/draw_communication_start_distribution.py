...
# 定义文件路径
from_files = os.path.join('./packet', 'from106.txt')
to_files = os.path.join('./packet', 'to106.txt')
...
# 定义文件路径
from_files = os.path.join('./packet', 'from108.txt')
to_files = os.path.join('./packet', 'to108.txt')
...
# 定义文件路径
from_files = os.path.join('./packet', 'from112.txt')
to_files = os.path.join('./packet', 'to112.txt')

# 尝试从文件中加载communication_start_time
if os.path.exists('./communication_start_distribute/communication_start_time_112.pkl'):
    with open('./communication_start_distribute/communication_start_time_112.pkl', 'rb') as f:
        communication_start_time = pickle.load(f)
else:
    # 遍历from*.txt文件
    for file in glob.glob(from_files):
    ...
    # 将communication_start_time保存到文件中
    with open('./communication_start_distribute/communication_start_time_112.pkl', 'wb') as f:
        pickle.dump(communication_start_time, f)
...
plt.xlabel('Start Time of Communication(s)')
plt.ylabel('Frequency')
plt.title('Distribution of start time of communication (Machine 112)')

# 显示图形
plt.savefig('./communication_start_distribute/112_distribution.png')
plt.close()